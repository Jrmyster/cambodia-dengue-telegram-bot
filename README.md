...
src/resilient-store.js   Storage failure classification and in-memory fallback
src/process-errors.js    Sanitized process error boundaries
...
`/readyz` also reports `transport`, the public `bot_username`, and `storage` without exposing credentials or user data. If Telegram is silent while health is green, confirm the username matches the intended bot, inspect the structured runtime errors, and check that only one poller is running for that token. A 404 at `/telegram/webhook` is expected in polling mode; webhook mode should reject unauthenticated requests with 403. Use Telegram's authenticated `getWebhookInfo` privately to investigate registration and delivery errors; never paste a bot token into public URLs or logs.
...
The host terminates HTTPS; Express listens on the host-provided `PORT`. The webhook checks Telegram's `X-Telegram-Bot-Api-Secret-Token` header with a timing-safe comparison before parsing JSON. It acknowledges after the session, update ID and reply commit in the active persistent or memory store, or after bilingual recovery guidance is sent for an unexpected handler error. If recovery delivery also fails, it returns 503 for Telegram to retry.

The Docker image prepares `/app/data` for the non-root `node` user; Render's disk is mounted there. At startup, the service opens the configured database and exercises a write transaction, which also checks access to SQLite's companion files. Actual mount permissions are deployment-specific. If the directory is inaccessible, the database is read-only/full/corrupt, or a write lock lasts beyond the one-second timeout, the service uses an in-memory SQLite store. `/readyz` reports `storage: "memory"` and logs emit `storage_memory_fallback`; it can still return 200 while serving conversations. Repair disk ownership/access as the runtime user and restart to restore persistent operation. `/tmp/bot.sqlite` can be used intentionally on an ephemeral host, but both `/tmp` and unmounted `./data` are lost on redeploy; the memory fallback does not silently create an alternate disk file.
...
- **Telegram reconnects:** Telegraf retries transient `getUpdates` failures internally. Initialization and polling launch also retry network timeouts/resets, 429s and server failures, logging only `telegram_retry` or `polling_retry` and the delay. Backoff increases from one to sixty seconds (or Telegram's longer `retry_after`). Polling 409 conflicts log `polling_conflict` and wait at least 30 seconds before reconnecting. This allows overlapping deploys to settle; persistent conflicts still require stopping the other poller or choosing one transport. Invalid credentials and configuration still fail visibly. The health server starts before Telegram API initialization.
- **Session recovery:** `/start` bypasses the burst cap, avoids reading old JSON, changes the session nonce, clears prior answers/location consent and replaces queued prompts with the language menu. A generic handler failure sends bilingual `/start` guidance independently of SQLite. Known storage/transport rejections are handled without terminating the process. Unknown unhandled rejections and uncaught exceptions log sanitized events and trigger controlled shutdown so Render can restart a clean process; Node advises against continuing normal operation after an uncaught exception.
- **Memory fallback limitations:** sessions, deduplication, reporting counts and the reply queue switch together and remain in memory until restart. They retain the same expiry/privacy rules but are not durable. Disk state cannot be reliably copied during a failure: existing disk sessions/replies become unavailable, users may need to restart, and reports collected in memory are lost at process exit. No automatic merge or switch back occurs. In-flight disk deliveries cannot remove or delay unrelated memory replies. The existing disk is left intact for repair; normal TTL pruning applies when it is opened again. Monitor fallback events because health alone does not indicate durable storage.
