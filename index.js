...
let server, store, bot, worker, ready = false, closing = false, polling = false;
...
try {
  ...
  ready = true;
  if (c.mode === 'webhook') {
    await bot.telegram.setWebhook(`${c.publicUrl}/telegram/webhook`, {
    ...
    void bot.launch({ dropPendingUpdates: false, allowedUpdates: ['message', 'callback_query'] }).catch(() => {
